...
ID3D11RasterizerState* DX11::RasterizerState;
...
ID3D11DepthStencilState* DX11::DepthEnabledState = nullptr;
ID3D11DepthStencilState* DX11::DepthDisabledState = nullptr;
bool DX11::myDepthCurrentlyEnabled = true;
...
bool DX11::Init(WindowsWindow* aWindowHandler)
{
	...
	//Create depth-stencil states (enabled + disabled)
	{
		...
		HRESULT hr = myDevice->CreateDepthStencilState(&desc, myDepthEnabledState.ReleaseAndGetAddressOf());
		assert(SUCCEEDED(hr));
		DepthEnabledState = myDepthEnabledState.Get();
	}

	{
		D3D11_DEPTH_STENCIL_DESC desc{};
		desc.DepthEnable = FALSE;//UI overlay: no depth test
		desc.DepthWriteMask = D3D11_DEPTH_WRITE_MASK_ZERO;
		desc.DepthFunc = D3D11_COMPARISON_ALWAYS;

		HRESULT hr = myDevice->CreateDepthStencilState(&desc, myDepthDisabledState.ReleaseAndGetAddressOf());
		assert(SUCCEEDED(hr));
		DepthDisabledState = myDepthDisabledState.Get();
	}
	...
	myDevice->CreateRasterizerState(&rasterizerDesc, myRasterizerState.ReleaseAndGetAddressOf());

	RasterizerState = myRasterizerState.Get();

	//default to enabled
	Context->OMSetDepthStencilState(DepthEnabledState, 0);
	myDepthCurrentlyEnabled = true;
...
bool DX11::ResizeToWindowSize()
{
	...
	//after ClearState/Resize, restore default depth state (enabled)
	if (Context && DepthEnabledState)
	{
		Context->OMSetDepthStencilState(DepthEnabledState, 0);
		myDepthCurrentlyEnabled = true;
	}
...
void DX11::SetDepthEnabled(bool aEnabled)
{
	if (!Context)
		return;

	if (aEnabled == myDepthCurrentlyEnabled)
		return;

	ID3D11DepthStencilState* state = aEnabled ? DepthEnabledState : DepthDisabledState;
	if (!state)
		return;

	Context->OMSetDepthStencilState(state, 0);
	myDepthCurrentlyEnabled = aEnabled;
}
