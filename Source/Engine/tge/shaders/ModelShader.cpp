...
{
	// todo: should only be created once, not once per shader!
...
void Tga::ModelShader::Render(const TextureResource* const* someTextures, const Model::MeshData& aModelData, const Matrix4x4f& aObToWorld, const Matrix4x4f* someBones) const
{
	...
	result = DX11::Context->Map(myBoneBuffer.Get(), 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedVResource);
	if (FAILED(result))
	{
	...
	DX11::Context->Unmap(myBoneBuffer.Get(), 0);
	DX11::Context->VSSetConstantBuffers((int)ConstantBufferSlot::Bones, 1, myBoneBuffer.GetAddressOf());

	result = DX11::Context->Map(myObjectBuffer.Get(), 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedVResource);
	if (FAILED(result))
	{
		//INFO_PRINT("Error in rendering!");
		return;
	...
	DX11::Context->PSSetConstantBuffers((int)ConstantBufferSlot::Object, 1, myObjectBuffer.GetAddressOf());
	...
	DX11::Context->RSSetState(DX11::RasterizerState);
